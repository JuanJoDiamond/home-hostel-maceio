...
| 2026-09-22 | Vista previa al compartir el link (Open Graph) | Revisado con opengraph.xyz tras la migración. Agregado `og:site_name` (pendiente desde el Sprint 1; Twitter Card ya existía desde el QA final) + `og:image:width`/`height`/`alt`. `og:description` y `twitter:description` acortadas a 99 caracteres (WhatsApp/mobile cortan ~125) con llamada a la acción a WhatsApp; la `meta description` para Google se mantiene en 142. La sugerencia de poner texto sobre la imagen se descartó por ahora -- ver el ítem "Imagen cabecera para compartir". Ver `MIGRACION-HOSTINGER.md` §8. |
